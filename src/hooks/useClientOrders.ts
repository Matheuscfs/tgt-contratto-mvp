...
import { BookingWithCompany, JobRequest, DbJobRequest, DbBooking } from '../types';
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<ClientOrdersData> => {
            ...
            const [jobsResponse, bookingsResponse] = await Promise.all([
                // 1. Fetch Jobs with Proposals and Categories
                supabase
                    .from('jobs')
                    .select(`
            *,
          ...
          `)
            ...
            const jobs: JobRequest[] = rawJobs.map((j) => ({
                ...
                category: j.categories ? { name: j.categories.name } : undefined,
