...
const CompanyCard: React.FC<CompanyCardProps> = ({ company }) => {
  ...
  return (
    <div className="group relative">
        <Link to={`/empresa/${company.slug}`} className="block bg-white border rounded-lg shadow-sm overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1 hover:ring-2 hover:ring-primary-500">
            <div className="relative w-full h-48 bg-gray-200">
                <img src={company.coverImage} alt={company.companyName} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent"></div>
                <img src={company.logo} alt={`${company.companyName} logo`} className="absolute bottom-2 left-2 w-16 h-16 rounded-full border-2 border-white bg-white shadow-md"/>
            </div>
            <div className="p-4">
                <p className="text-xs font-semibold text-primary-600 uppercase tracking-wide">{company.category}</p>
                <p className="block mt-1 text-lg font-semibold text-gray-900 group-hover:text-primary-600 transition-colors">{company.companyName}</p>
                <div className="mt-2 flex items-center">
                <div className="flex items-center">
                    {[...Array(5)].map((_, i) => (
                    <svg key={i} className={`w-4 h-4 ${i < Math.round(company.rating) ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
